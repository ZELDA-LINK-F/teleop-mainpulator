"""Mavis 发送：v16 修正 cut 方向逻辑

v15 设计错误：
- 全部用 normal X- plane
- 但 B2 输入关节孔在 X=1.5（板 X- 端），应该切 X+ 方向（normal X+）
- B3 输入关节孔在 X=4.5（板 X- 端），应该切 X+ 方向（normal X+）

正确方向规则（关键）：
- B1 输出轴孔：板 X(-1.5, 1.5), 在 X=1.5 端面 -> 切 X- 方向 (normal X-)
- B2 输入关节孔：板 X(1.5, 4.5), 在 X=1.5 端面 -> 切 X+ 方向 (normal X+)
- B2 输出关节孔：板 X(1.5, 4.5), 在 X=4.5 端面 -> 切 X- 方向 (normal X-)
- B3 输入关节孔：板 X(4.5, 7.0), 在 X=4.5 端面 -> 切 X+ 方向 (normal X+)
"""
import json, uuid
...
def cut_hole_at_x(body, x_pos, y_pos, z_pos, r, normal_dir, name, dist=0.3):
    ...
    normal_dir: +1 = cut 沿 X+ 方向（板内方向）, -1 = cut 沿 X- 方向
    """
...
data = {
    ...
    "code": code,
    "description": "v16 修正 cut 方向（4 个关节孔各有正确 normal）"
}

with open(r"D:\ESP\遥操作机械\fusion-ai-assistant\queue\mavis_command.json", "w", encoding="utf-8") as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print(f"v16 已直接写入 mavis_command.json! command_id: {data['command_id']}")