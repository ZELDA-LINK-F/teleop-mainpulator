"""Mavis 发送：指段 1 v7（2 个独立草图避免 2D 投影 bug）
- 步骤 0：清空
- 步骤 1：草图1 外轮廓 30x16 → 拉伸 25mm → 板
- 步骤 2：草图2 内 hole 24x14 → cut 板
- 步骤 3：草图3 (XZ平面) 输出轴孔 (1.3, 0, 1.4) → cut 板
- 步骤 4a：草图4a (XY平面) 1 个圆 (0, 0.75, 0) → cut 板（独立草图）
- 步骤 4b：草图4b (XY平面) 1 个圆 (0, -0.75, 0) → cut 板（独立草图）
- 步骤 5：草图5 (XY平面) 4 个手掌螺丝孔 → cut 板
- 步骤 6：自动 verify
"""
import json, uuid
...
try:
    ...
    result["steps"].append(f"步骤1: 板 30x16x25, V={body1.volume:.2f}")
    ...
    # 4a. 草图4a 独立 XY 平面 1 个圆 (0, 0.75, 0) → cut Y+ SG90 螺丝
    ...
    try:
        ...
        result["warnings"].append(f"Y+ SG90 螺丝: {str(e)[:80]}")
    result["steps"].append(f"步骤4a: Y+ SG90 螺丝, V={body1.volume:.2f}")

    # 4b. 草图4b 独立 XY 平面 1 个圆 (0, -0.75, 0) → cut Y- SG90 螺丝
    ...
    try:
        ...
        result["warnings"].append(f"Y- SG90 螺丝: {str(e)[:80]}")
    result["steps"].append(f"步骤4b: Y- SG90 螺丝, V={body1.volume:.2f}")
