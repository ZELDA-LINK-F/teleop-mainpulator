"""Mavis 发送：v11 完整手指骨架 - 3 个指段 + 关节孔

设计要点（从 v5-v10 学到的教训）：
1. 输出轴孔用 face 作为草图平面（不在 X=0 平面画！v9 bug）
2. 单向 cut（避免双向 5cm 触发 Fusion 2702 bug）
3. 每个 cut 独立草图（避免 max profile 选错）
4. 3 个 body 错开 X 位置（不重叠）
5. SG90 螺丝手动 Fusion 后切（v9 SG90 螺丝触发 bug，先省略）

指段 1 (基段): X(-1.5, 1.5), 30x16x25, 装 SG90-A, 4 手掌螺丝
指段 2 (中段): X( 1.5, 4.5), 30x16x25, 装 SG90-B, 2 关节孔
指段 3 (指尖): X( 4.5, 7.0), 25x16x20, 无 SG90, 1 关节孔
"""
import json, uuid
...
def find_face_at_x(body, x_pos, tol=0.01):
    ...
    for face in body.faces:
        ...
        if abs(bbox.maxPoint.x - x_pos) < tol or abs(bbox.minPoint.x - x_pos) < tol:
            # 确保这是个平面（不是曲面）
            if face.geometry.objectType == adsk.core.Plane.classType() or \
               hasattr(face.geometry, 'normal'):
                # 检查 face 法向是否是 X 方向
                try:
                    normal = face.geometry.normal
                    if abs(normal.x) > 0.9 and abs(normal.y) < 0.1 and abs(normal.z) < 0.1:
                        return face
                except:
                    pass
    return None

def cut_hole_on_x_face(body, x_pos, y_pos, z_pos, r, name, dist=0.5):
    """在 body X=x_pos 端面画圆并 cut 单向"""
    face = find_face_at_x(body, x_pos)
    if face is None:
        result["warnings"].append(f"{name}: 没找到 X={x_pos} 端面 face")
        return False
...
try:
    ...
    # ==================== Body 1: 指段 1 (基段) ====================
    # X(-1.5, 1.5), Y(-0.8, 0.8), Z(0, 2.5) = 30x16x25 mm
    ...
    # B1 内 hole 24x14 (X=-1.2~1.2, Y=-0.7~0.7), Z 贯穿
    ...
    cut2.setDistanceExtent(False, adsk.core.ValueInput.createByReal(2.5))  # 单向 Z+
    try:
        extrudes.add(cut2)
    ...
    # B1 输出轴孔 (X+ 端面 X=1.5, 圆心 3D(1.5, 0, 2.25))
    if cut_hole_on_x_face(body1, 1.5, 0, 2.25, 0.25, "B1 输出轴孔", 0.5):
    ...
    # ==================== Body 2: 指段 2 (中段) ====================
    # X(1.5, 4.5), Y(-0.8, 0.8), Z(0, 2.5) = 30x16x25 mm
    ...
    # B2 内 hole 24x14 (X=1.8~4.2, Y=-0.7~0.7)
    ...
    # B2 输入关节孔 (X- 端面 X=1.5, 圆心 3D(1.5, 0, 2.25))
    if cut_hole_on_x_face(body2, 1.5, 0, 2.25, 0.25, "B2 输入关节孔", 0.5):
        result["steps"].append(f"B2 输入关节孔: V={body2.volume:.2f}")

    # B2 输出关节孔 (X+ 端面 X=4.5, 圆心 3D(4.5, 0, 2.25))
    if cut_hole_on_x_face(body2, 4.5, 0, 2.25, 0.25, "B2 输出关节孔", 0.5):
        result["steps"].append(f"B2 输出关节孔: V={body2.volume:.2f}")

    # ==================== Body 3: 指段 3 (指尖) ====================
    # X(4.5, 7.0), Y(-0.8, 0.8), Z(0, 2.0) = 25x16x20 mm
    ...
    # B3 输入关节孔 (X- 端面 X=4.5, 圆心 3D(4.5, 0, 2.25))
    if cut_hole_on_x_face(body3, 4.5, 0, 2.25, 0.25, "B3 输入关节孔", 0.5):
    ...
    if n == 3:
        ...
        # B1 期望 30x16x25, B2 期望 30x16x25, B3 期望 25x16x20
