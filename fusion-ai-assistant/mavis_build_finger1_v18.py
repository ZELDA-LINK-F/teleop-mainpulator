"""Mavis 发送：v18 用 ConstructionPlane 中转创建任意平面

v17 失败原因：
- sketches.add(adsk.core.Plane) -> "not a planar entity"
- adsk.core.Plane 不是 sketches.add 期望的类型

v18 解决（从 Autodesk Community 帖子学到的）：
1. 先用 adsk.core.Plane.create(origin, normal) 创建几何 Plane
2. 用 rootComp.constructionPlanes.add() 创建 ConstructionPlane（Fusion 实际对象）
3. 用 sketches.add(constructionPlane)
"""
import json, uuid
...
def cut_hole_at_x(body, x_pos, y_pos, z_pos, r, normal_dir, name, dist=0.3):
    """用 ConstructionPlane 在 body X=x_pos 处切 ⌀r 圆柱孔
    normal_dir: +1 = cut 沿 X+ 方向, -1 = cut 沿 X- 方向
    """
    # 1. 创建几何 Plane
    geo_plane = adsk.core.Plane.create(
        adsk.core.Point3D.create(x_pos, 0, 0),
        adsk.core.Vector3D.create(normal_dir, 0, 0)
    )
    
    # 2. 创建 ConstructionPlane
    plane_input = rootComp.constructionPlanes.createInput()
    plane_input.setByPlane(geo_plane)
    construction_plane = rootComp.constructionPlanes.add(plane_input)
    
    # 3. 在 ConstructionPlane 上创建 sketch
    sk = rootComp.sketches.add(construction_plane)
...
try:
    ...
    # B1 输出轴孔：X=1.5 plane, normal X- (切进板)
    ...
    # B3 输入关节孔：X=4.5 plane, normal X+ (切进板)
