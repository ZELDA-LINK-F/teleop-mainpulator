...
def find_face_at_x(body, x_pos, tol=0.05):
    """找 body 上 X=x_pos 处的端面 face"""
    candidates = []
    for f in body.faces:
        bbox = f.boundingBox
        # X 端面：bbox X 范围是单点（厚度 < 0.1cm）
        x_thickness = abs(bbox.maxPoint.x - bbox.minPoint.x)
        if x_thickness > 0.1:
            continue
        # 检查 X 坐标匹配
...
def cut_hole_at_x(body, x_pos, y_pos, z_pos, r, name):
    ...
    candidates = find_face_at_x(body, x_pos)
    if not candidates:
    ...
    for i, f in enumerate(candidates):
        ...
        try:
            ...
            geom_type = f.geometry.objectType
            result["steps"].append(f"  {name} 候选{i}: bbox X=[{bbox.minPoint.x:.3f}, {bbox.maxPoint.x:.3f}], normal=({normal.x:.2f},{normal.y:.2f},{normal.z:.2f}), geom_type={geom_type}")
        except Exception as e:
            result["steps"].append(f"  {name} 候选{i}: bbox X=[{bbox.minPoint.x:.3f}, {bbox.maxPoint.x:.3f}], no normal ({str(e)[:50]})")
    
    # 选择法向 X+ 的 face（X+ 端面或内孔 X+ 壁），如果没有就用第一个
    face = candidates[0]
    for f in candidates:
        try:
            normal = f.geometry.normal
    ...
    # 关键：cut 方向应该沿 -face 法向（切进板内）
    if normal.x > 0:
        dist = -0.5  # X+ face -> X- direction
    else:
        dist = 0.5   # X- face -> X+ direction
    ...
    try:
        ...
        err_msg = str(e)
        result["warnings"].append(f"{name}: {err_msg[:300]}")
        return False
...
try:
    ...
    # B1 输出轴孔
    result["steps"].append(f"--- B1 输出轴孔调试 ---")
    ...
    result["steps"].append(f"--- B3 输入关节孔调试 ---")
    ...
    if n == 3:
        ...
        result["success"] = True
        result["judgment"] = f"3 个 body 创建成功"
    else:
